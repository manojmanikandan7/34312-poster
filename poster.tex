...
\begin{exampleblock}{Nesterov's accelerated gradient descent}
    ...
    \begin{itemize}
      ...
      %
      \item To obtain the telescopic sum, denote
      $v_t := \tfrac{\beta}{2}\norm{y_t + \lambda_t d_t - x^*}^2$.
      We need:
      \begin{equation*}
        y_t + \lambda_t(d_t + g_t)
          \;=\; y_{t+1} + \lambda_{t+1}\,d_{t+1}
      \end{equation*}
      Since $y_{t+1} = y_t + d_t + g_t$ and
      ...
      % --- SELECTING lambda_t ---
      %
      \item With~(\ref{eq:dagger}) now telescoping as
      $\lambda_t^2\,\delta_{t+1} - (\lambda_t^2 {-} \lambda_t)\,\delta_t
      \leq v_t - v_{t+1}$, we require
