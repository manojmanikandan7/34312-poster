...
\begin{lemma}[\bf 1]
Let $f:\mathbb{R}^n \rightarrow \mathbb{R}^n$ be a convex and $\beta$-smooth function. Let $x, y \in \mathbb{R}^n$ and $x^+ = x - \frac{1}{\beta}\nabla f(x)$.
...
\begin{exampleblock}{Main Ideas for Theorem $2$}
\begin{itemize}
  \item Nesterov's accelerated gradient descent uses the ideas from classical momentum gradient descent (cite), the gradient is computed at $x_t$ to produce $y_{t+1}$ but 
  the next $x_{t+1}$ is not simply $y_{t+1}$. $x_{t+1}$ overshoots $y_{t+1}$ in the direction of the most recent step $y_t$ with some ``momentum'' given by $(1 - \gamma_t)$ and $\gamma_t$ 
  on $y_{t+1}$ and $y_t$ respectively.
  \item The sequence $\lambda_t$ comes from the need to satisfy $\lambda_{t-1}^2 = \lambda_{t}^2 - \lambda_t$. 
\end{itemize}
\end{exampleblock}
